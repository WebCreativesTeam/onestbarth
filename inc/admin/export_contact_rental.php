<?php		
	...
	header("Content-Disposition: attachment; filename=contact-rental-".$today.".xls");
	header("Pragma: no-cache");
	header("Expires: 0");

	$res=$wpdb->get_results( "SELECT * FROM `osb_contact_rental` ORDER BY `date_saving` DESC");
	/*echo '<pre>';
	print_r($res);	
	echo '</pre>';
	*/
