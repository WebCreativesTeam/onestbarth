<?php		
	...
	header("Content-Disposition: attachment; filename=transfert-request-".$today.".xls");
	header("Pragma: no-cache");
	header("Expires: 0");

	$res=$wpdb->get_results( "SELECT * FROM `osb_transfert` ORDER BY `date_saving` DESC");
	/*echo '<pre>';
	print_r($res);	
	echo '</pre>';
	*/
