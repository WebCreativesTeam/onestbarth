<?php get_header(); ?>
<?php
	$collection =$wp_query->queried_object;
	$imgCollection=get_field('collection_image', 'collection_'.$collection->term_id);
	
	$LinksFooterCollection=get_field('rental_links', 'collection_'.$collection->term_id);
	...
	$listC = get_terms('collection');
	$i=0;
	foreach($listC as $key => $oC){
		if($oC->term_id == $collection->term_id){
			echo $oC->term_id;
			$previd=$i-1;
			$nextid=$i+1;
		}
...
<div id="listing" class="listing">
	...
	<div id="listing-header" style="background-image:url(<?php echo $imgCollection['url']; ?>);">
		<div id="listing-header-content">
			<h1><?php echo strtoupper($collection->name); ?></h1>
			<p>
		...
		<?php
			...
			{
				echo '<a href="'.get_term_link($listC[$previd]).'" ttile="'.$listC[$previd]->name.'" class="link-collection" id="link-collection-prev"><i class="fa fa-angle-left"></i></a>';
			}
			
			if(isset($nextid) && $nextid < count($listC))
			{
				echo '<a href="'.get_term_link($listC[$nextid]).'" ttile="'.$listC[$nextid]->name.'" class="link-collection" id="link-collection-next"><i class="fa fa-angle-right"></i></a>';
			}
		?>
	</div>
			...
			<div class="container">
				...
				<div id="listing-villa-footer">
					<div id="listing-villa-footer-left">
						<?php
							foreach($LinksFooterCollection as $key => $oL)
							{
